...
def edgeInference(fileImg, p_tar, nr_branch_edge):
	...
	print("****************************************************")
	print(output, conf_list, class_list, isTerminate)
	if (not isTerminate and False):
	...
	response_request["inference_time"]=inference_time
	response_request["isTerminate"] = "Yes" if isTerminate else "No"
	response_request["nr_branches_model"] = model.nr_branches_model
	response_request["conf_list"] =conf_list.tolist() if type(conf_list) is not list else conf_list
	response_request["class_list"] =class_list.tolist() if type(class_list) is not list else class_list
	print("response_request:",response_request)
