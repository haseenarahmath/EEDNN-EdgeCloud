...
class Early_Exit_DNN(nn.Module):
  def __init__(self, model_name: str, n_classes: int, 
    ...
    self._temperature_overall = None
  ...
  @property
  def temperature_overall(self):
    return self._temperature_overall

  @temperature_overall.setter  
  def temperature_overall(self, temp_overall):
    self._temperature_overall = temp_overall

  def temperature_scale_overall(self, logits):
    temperature = self._temp_overall.unsqueeze(1).expand(logits.size(0), logits.size(1)).to(self.device)
  ...
  def forwardEdgeInference(self, x, p_tar, nr_branch_edge):
    ...
    return x, conf_list, class_list, isTerminate
  ...
  def forward(self, x, p_tar=0.5, training=False):
    """
