...
def load_model(device):
	...
	"""self.b_model = Early_Exit_DNN(config.model_name, config.n_classes, config.pretrained, config.nr_branch_model,
		config.input_shape, config.exit_type, device, distribution=config.distribution)
	b_mobilenet_model.load_state_dict(torch.load(config.model_path_edge_final, map_location=device)["model_state_dict"])
...
class ModelLoad():
	...
	def update_load_model(self, device, dataset_name):
		...
		self.b_model = Early_Exit_DNN(config.model_name, config.n_classes, config.pretrained, self._nr_branches_model, 
			config.input_shape, config.exit_type, device, distribution=config.distribution)

		model_path = os.path.join(config.edge_model_path, "mobilenet_%s_2_branches_%s.pth"%(dataset_name, self._nr_branches_model))
		#model_path = os.path.join(config.edge_model_path,"cifar10/pristine_model_mobilenet_cifar10_3_5_b.pth")

		#self.b_model.load_state_dict(torch.load(model_path, map_location=device)["model_state_dict"])
		self.b_model.load_state_dict(
			torch.load(model_path, map_location=device, weights_only=False)["model_state_dict"])

	def load_temperature(self):
		temp_path = os.path.join(config.temp_edge_path, "branches_temp_scaling_branches_%s_2.csv"%(self._nr_branches_model))
